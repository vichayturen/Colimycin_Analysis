...
output_file = r"C:\Users\Administrator\Desktop\可利霉素分析结果\基线情况\1008基线情况汇总_v2填充数据1013_实验组检验.xlsx"
time_props = [
    "WBC", "HB", "PLT", "N", "L", "MONO", "HCT", "ALT", "AST", "TB", "DB", "ALB", "G", "BUN", "Cr",
    "K", "PT", "Cl", "Ca", "INR", "APTT", "TT", "PCT", "CRP", "血沉", "IL6"]
timing = ["入院或入ICU时间.1", "可利霉素前24小时", "可D1-2", "可后D3-D4", "可后D5-D6", "可后D7-D8"]
timing_postfix = ["", ".1", ".2", ".3", ".4", ".5"]
# timeing = ["before", "3", "5", "7", "stop", "discharge"]

time_format = "%Y/%m/%d"
na = "NA"
df1 = pd.read_excel(input_file, sheet_name="Sheet3")
...
def get_result(
    ...
    sql = f"""select * from check_record
              where `患者编号`='{id}' {check_item_rule} {check_time_rule} order by `采集时间`"""
    if desc:
        sql += " desc"
    df = pd.read_sql(sql, con=engine)
    if len(df) > 0:
        print(df.loc[0, '检验子项结果'])
    ...
    for t, postfix in zip(timing, timing_postfix):
        ...
        for index, row in df1.iterrows():
            if pd.isna(df1.loc[index, col]):
                desc = False
                if t == "入院或入ICU时间.1":
                    from_day = df1.loc[index, "入院或入ICU时间.1"]
                    ...
                    from_day = df1.loc[index, "可后D3-D4"]
                    to_day = df1.loc[index, "可后D3-D4"] + pd.Timedelta(days=1)
                elif t == "可后D5-D6":
                    from_day = df1.loc[index, "可后D5-D6"]
                    to_day = df1.loc[index, "可后D5-D6"] + pd.Timedelta(days=1)
                elif t == "可后D7-D8":
                    from_day = df1.loc[index, "可后D7-D8"]
                    to_day = df1.loc[index, "可后D7-D8"] + pd.Timedelta(days=1)
                else:  # 出院
                    print("未知时间段")
                    continue
                df1.loc[index, col] = get_result(df1.loc[index, '患者编号'], prop, from_day=from_day, to_day=to_day, desc=desc)
            pbar.update()
        pbar.close()
