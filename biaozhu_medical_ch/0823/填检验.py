...
df1 = pd.read_excel('/home/vv/文档/xwechat_files/wxid_vlh3ec6ed7tx22_8772/msg/file/2024-08/baseline可利霉素.xls',
                    sheet_name='检验1')
...
for row_index, row in df1.iterrows():
    ...
    jixianshijian = get_time(row['基线时间'])
    keqian = get_time(row['可利霉素前24小时'])
    ked1 = get_time(row['可D1'])
    ked3 = get_time(row['可后D3-D4'])
    ked5 = get_time(row['可后D5-D6'])
    ked7 = get_time(row['可后D7-D8'])
    for prop in props:
        df1.loc[row_index, prop] = get_result(id, prop, jixianshijian, keqian+pd.Timedelta(days=-2))
        df1.loc[row_index, prop+'.1'] = get_result(id, prop, keqian+pd.Timedelta(days=-1), keqian, True)
        df1.loc[row_index, prop+'.2'] = get_result(id, prop, ked1, ked1+pd.Timedelta(days=1))
        df1.loc[row_index, prop+'.3'] = get_result(id, prop, ked3, ked3+pd.Timedelta(days=1))
        df1.loc[row_index, prop+'.4'] = get_result(id, prop, ked5, ked5+pd.Timedelta(days=1))
        df1.loc[row_index, prop+'.5'] = get_result(id, prop, ked7, ked7+pd.Timedelta(days=1))
    pbar.update()
pbar.close()

df1.to_excel("/home/vv/文档/xwechat_files/wxid_vlh3ec6ed7tx22_8772/msg/file/2024-08/baseline可利霉素_检验1.xlsx", index=False)
