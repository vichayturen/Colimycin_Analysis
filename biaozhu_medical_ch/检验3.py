...
for i, row in df.iterrows():
            ...
            for pattern in col_pattern_map:
                if col.startswith(pattern):
                    ...
                    if pattern == "新冠":
                        ...
                        order by `采集时间`"""
                        tmp_df = pd.read_sql(sql=sql, con=engine)
                        tmp_result = json.dumps(
                            {tmp_df.loc[i, "检验子项中文名"]: [tmp_df.loc[i, "检验子项结果"], str(tmp_df.loc[i, "采集时间"])]
                            for i in range(len(tmp_df))},
                            ensure_ascii=False,
                            indent=4) if len(tmp_df) > 0 else None
                    else:
                        sql = f"""select * from check_record
                        where `患者编号`={row['患者编号']}
                        {check_name_rule}
    ...
    #     break
pbar.close()
df.to_excel("tmp.xlsx", sheet_name=sheet, index=False, na_rep="NA")
